...
public class RawDataFilesEditor extends HBox implements ParameterEditor<RawDataFilesSelection> {
  ...
  public RawDataFilesEditor(PropertySheet.Item parameter) {
    ...
    typeCombo = new ComboBox<>(
        FXCollections.observableList(Arrays.asList(RawDataFilesSelectionType.values())));
    ...
    detailsButton.setOnAction(e -> {
      ...
      if (type == RawDataFilesSelectionType.SPECIFIC_FILES) {
        ...
        final @Nonnull List<RawDataFile> allFiles =
            MZmineCore.getCurrentProject().getRawDataFiles();
        final MultiChoiceParameter<RawDataFile> filesParameter =
            new MultiChoiceParameter<RawDataFile>("Select files", "Select files", "Files", allFiles,
                specificFiles);
        ...
        if (exitCode == ButtonType.OK) {
          specificFiles = paramSet.getParameter(filesParameter).getValue();
        }
  ...
  public RawDataFilesSelection getValue() {
    RawDataFilesSelectionType selectionType = typeCombo.getSelectionModel().getSelectedItem();
    if (selectionType == null)
      selectionType = RawDataFilesSelectionType.ALL_FILES;
    return new RawDataFilesSelection(selectionType, specificFiles, namePattern);
  }
