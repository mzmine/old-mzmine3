...
public class OptionalModuleParameter extends AbstractParameter<Boolean> {
  ...
  public OptionalModuleParameter(@Nonnull String name, @Nonnull String description,
      @Nonnull String category, ParameterSet embeddedParameters) {
    ...
    setValidator((currentValue, messages) -> {
      ...
      if (currentValue) {
        ...
        for (Parameter<?> embeddedParam : embeddedParameters.getParameters()) {
          ParameterValidator embeddedValidator = embeddedParam.getValidator();
          if (embeddedValidator == null)
            continue;
          Object value = embeddedParam.getValue();
          boolean result = embeddedValidator.checkValue(value, voidMsgs);
          if (!result) {
  ...
  public @Nonnull OptionalModuleParameter clone() {
    ...
    final OptionalModuleParameter copy = new OptionalModuleParameter(getName(), getDescription(),
        getCategory(), embeddedParametersClone);
