...
public class DraggableTab extends Tab {
    ...
    public DraggableTab(String text) {
        ...
        nameLabel.setOnMouseDragged(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent t) {
                ...
                dragStage.show();
                Point2D screenPoint = new Point2D(t.getScreenX(),
                        t.getScreenY());
                ...
                if (data == null || data.getInsertPane().getTabs().isEmpty()) {
                    ...
                    Rectangle2D rect = getAbsoluteRect(
                            data.getInsertPane().getTabs().get(index));
                    if (end) {
        ...
        nameLabel.setOnMouseReleased(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent t) {
                ...
                if (!t.isStillSincePress()) {
                    ...
                    int oldIndex = oldTabPane.getTabs()
                            .indexOf(DraggableTab.this);
                    ...
                    if (insertData != null) {
                        ...
                        insertData.getInsertPane().getTabs().add(addIndex,
                                DraggableTab.this);
                    ...
                    pane.getTabs().addListener(new ListChangeListener<Tab>() {
                        ...
                        public void onChanged(
                                ListChangeListener.Change<? extends Tab> change) {
        ...
        for (TabPane tabPane : tabPanes) {
            ...
            if (tabAbsolute.contains(screenPoint)) {
                ...
                if (!tabPane.getTabs().isEmpty()) {
                    Rectangle2D firstTabRect = getAbsoluteRect(
                            tabPane.getTabs().get(0));
                    if (firstTabRect.getMaxY() + 60 < screenPoint.getY()
                            || firstTabRect.getMinY() > screenPoint.getY()) {
                        return null;
                    }
                    Rectangle2D lastTabRect = getAbsoluteRect(tabPane.getTabs()
                            .get(tabPane.getTabs().size() - 1));
                    if (screenPoint.getX() < (firstTabRect.getMinX()
                        ...
                        for (int i = 0; i < tabPane.getTabs().size() - 1; i++) {
                            ...
                            if (leftTab instanceof DraggableTab
                                ...
                                Rectangle2D leftTabRect = getAbsoluteRect(
                                        leftTab);
                                Rectangle2D rightTabRect = getAbsoluteRect(
                                        rightTab);
    ...
    private Rectangle2D getAbsoluteRect(Control node) {
        return new Rectangle2D(
                node.localToScene(node.getLayoutBounds().getMinX(),
                        node.getLayoutBounds().getMinY()).getX()
                + node.getScene().getWindow().getX(), node
                        .localToScene(node.getLayoutBounds().getMinX(),
