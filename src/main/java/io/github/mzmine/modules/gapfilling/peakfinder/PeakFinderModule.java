...
public class PeakFinderModule implements MZmineProcessingModule {
  ...
  private static final String MODULE_DESCRIPTION =
      "This module fills the missing gaps in the feature list by searching for a peak in the original raw data.";
  ...
  public void runModule(@Nonnull MZmineProject project, @Nonnull ParameterSet parameters,
    ...
    final FeatureTablesSelection featureTables =
        parameters.getParameter(PeakFinderParameters.featureTables).getValue();
    final MaximumMzTolerance mzTolerance =
        parameters.getParameter(PeakFinderParameters.mzTolerance).getValue();
    final RTTolerance rtTolerance =
        parameters.getParameter(PeakFinderParameters.rtTolerance).getValue();
    final Boolean isIntensityTolSet =
        parameters.getParameter(PeakFinderParameters.intensityTolerance).getValue();
    Double intensityTolerance = parameters.getParameter(PeakFinderParameters.intensityTolerance)
        .getEmbeddedParameter().getValue();
    final String nameSuffix = parameters.getParameter(PeakFinderParameters.nameSuffix).getValue();
    final Boolean removeOldTable =
        parameters.getParameter(PeakFinderParameters.removeOldTable).getValue();

    if (featureTables == null || featureTables.getMatchingFeatureTables().isEmpty()) {
      logger.warn("Peak finder module started with no feature table selected");
      return;
    }
    ...
    for (FeatureTable featureTable : featureTables.getMatchingFeatureTables()) {
      ...
      GapFillingMethod method = new GapFillingMethod(featureTable, dataStore, mzTolerance,
          rtTolerance, intensityTolerance, nameSuffix);

      MSDKTask newTask = new MSDKTask("Gap filling feature table", featureTable.getName(), method);
