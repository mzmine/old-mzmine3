...
public class MzTabExportModule implements MZmineProcessingModule {
    ...
    public void runModule(@Nonnull MZmineProject project,
        ...
        final FeatureTablesSelection featureTables = parameters
                .getParameter(MzTabExportParameters.featureTables).getValue();
        final File exportFilePattern = parameters
                .getParameter(MzTabExportParameters.exportFile).getValue();
        final Boolean exportAllFeatures = parameters
                .getParameter(MzTabExportParameters.exportAllFeatures)
                .getValue();
        ...
        if (exportFilePattern == null) {
            MZmineGUI.displayMessage(
                    "The path and name of the mzTab output file cannot be empty.");
            logger.warn(
                    "The path and name of the mzTab output file cannot be empty.");
            return;
        }
        ...
        for (FeatureTable featureTable : featureTables
            ...
            MzTabFileExportMethod method = new MzTabFileExportMethod(
                    featureTable, exportFile, exportAllFeatures);
