...
public class JoinAlignerModule implements MZmineProcessingModule {
  ...
  private static final String MODULE_DESCRIPTION =
      "This module aligns features using a match score calculated based on the mass and retention time of each feature.";
  ...
  public void runModule(@Nonnull MZmineProject project, @Nonnull ParameterSet parameters,
    ...
    final FeatureTablesSelection featureTables =
        parameters.getParameter(JoinAlignerParameters.featureTables).getValue();
    final MaximumMzTolerance mzTolerance =
        parameters.getParameter(JoinAlignerParameters.mzTolerance).getValue();
    final RTTolerance rtTolerance =
        parameters.getParameter(JoinAlignerParameters.rtTolerance).getValue();
    Integer mzWeight = parameters.getParameter(JoinAlignerParameters.mzWeight).getValue();
    Integer rtWeight = parameters.getParameter(JoinAlignerParameters.rtWeight).getValue();
    Boolean requireSameAnnotation =
        parameters.getParameter(JoinAlignerParameters.requireSameAnnotation).getValue();
    Boolean requireSameCharge =
        parameters.getParameter(JoinAlignerParameters.requireSameCharge).getValue();
    final String featureTableName =
        parameters.getParameter(JoinAlignerParameters.featureTableName).getValue();
    ...
    final Boolean removeOldTable =
        parameters.getParameter(JoinAlignerParameters.removeOldTable).getValue();

    if (featureTables == null || featureTables.getMatchingFeatureTables().isEmpty()) {
      logger.warn("Match aligner module started with no feature table selected");
      return;
    }
    if (featureTables.getMatchingFeatureTables().size() == 1) {
      logger.warn("Match aligner module started with less than two feature table");
      return;
    }
    ...
    JoinAlignerMethod method = new JoinAlignerMethod(featureTables.getMatchingFeatureTables(),
        dataStore, mzTolerance, rtTolerance, mzWeight, rtWeight, requireSameCharge,
        requireSameAnnotation, featureTableName);

    MSDKTask newTask = new MSDKTask("Aligning feature tables", featureTableName, method);
