...
public class GoogleAnalyticsTracker implements Runnable {
    ...
    public void run() {
        ...
        Boolean sendStatistics = MZmineCore.getConfiguration()
                .getSendStatistics();
        ...
        if (sendStatistics) {
            ...
            url.append("&utmcc=__utma%3D'" + cookie + "." + randomValue + "."
                    + now + "." + now + "." + now + ".2%3B%2B__utmb%3D" + cookie
                    + "%3B%2B__utmc%3D" + cookie + "%3B%2B__utmz%3D" + cookie
                    + "." + now
